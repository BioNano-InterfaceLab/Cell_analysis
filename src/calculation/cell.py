...
class Cell:
    ...
    def generateVelGraph(self):
        ...
        if 7 <= window_length:    
            ...
            xhat = scipy.signal.savgol_filter(diff, window_length, window_length-1)
    ...
    def saveImage(self):
        ...
        try:
            ...
            if (  self.firstCrop.shape[0] !=  self.firstCrop.shape[1] ):
                blackImage = np.zeros((abs(self.firstCrop.shape[1] -  self.firstCrop.shape[0]),  self.firstCrop.shape[1], 3), float)
